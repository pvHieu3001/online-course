  ...
  return (
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 relative'>
      {listVouchers?.data?.map((item: any, index: number) => {
        return (
          <div key={index} className={`flex flex-col p-5 rounded-2xl bg-red-50 dark:bg-opacity-90`}>
            ...
            <div className='mt-2.5'>
              ...
              <p className='text-slate-500 mt-0.5 text-sm'>
                Còn: <span className='text-red-500'> {item.quantity - item.used_count} </span> phiếu
              </p>
            </div>
          </div>
        )
