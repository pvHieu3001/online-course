...
import CancelAnimationIcon from '@/page/components/icon/OrderIcon/Cancel'
import SuccessAnimationIcon from '@/page/components/icon/OrderIcon/Success'
import OrderAnimationIcon from '@/page/components/icon/OrderIcon/Order'
export default function ListOrder() {
  const [changeStatus] = useChangeStatusOrderMutation()
  const { searchText, setSearchText, setSearchedColumn, searchedColumn, searchInput, handleSearch, handleReset } =
    useQuerySearch()
  ...
  const columns: TableProps<IBanner>['columns'] = [
    ...
    },
    {
      ...
      align: 'center',
      width: 160,
      render: (text) => <a>{formatTimestamp(text)}</a>
    },
    {
      title: 'Khách hàng',
        ...
        return (
          <div className='w-[150px]'>
            <Select
              ...
              style={{ width: 150 }}
              onChange={(e) => {
                handleOnChangeStatus(Number(e), Number(item.id))
              }}
              options={[
                {
                  value: '1',
                  label: <Tag color='red'>Chờ xử lý</Tag>,
                  disabled: handleDisableSelect(1, Number(item.order_status_id))
                },
                {
                  value: '2',
                  label: <Tag color='orange'>Đang chuẩn bị</Tag>,
                  disabled: handleDisableSelect(2, Number(item.order_status_id))
                },
                {
                  value: '3',
                  label: <Tag color='green'>Đơn hàng sẵn sàng</Tag>,
                  disabled: handleDisableSelect(3, Number(item.order_status_id))
                },
                {
                  value: '4',
                  label: <Tag color='cyan'>Đang vận chuyển</Tag>,
                  disabled: handleDisableSelect(4, Number(item.order_status_id))
                },
                {
                  value: '5',
                  label: <Tag color='blue'> Đang giao hàng</Tag>,
                  disabled: handleDisableSelect(5, Number(item.order_status_id))
                },
                {
                  value: '6',
                  label: <Tag color='geekblue'> Đã giao hàng</Tag>,
                  disabled: handleDisableSelect(6, Number(item.order_status_id))
                },
                {
                  value: '7',
                  label: <Tag color='success'> Hoàn thành</Tag>,
                  disabled: handleDisableSelect(7, Number(item.order_status_id))
                },
                {
                  value: '8',
                  label: <Tag color='#000000'> Đơn hàng bị hủy</Tag>,
                  disabled: handleDisableSelect(8, Number(item.order_status_id))
                }
              ]}
            />
          </div>
        ...
        <Row gutter={16} className='my-4'>
          ...
          <Col span={8}>
            <Card
              ...
              bordered={false}
              extra={<SuccessAnimationIcon width={70} height={70} />}
            >
              <div className='text-[25px]'>
                <b> 0 </b>
              </div>
          ...
          <Col span={8}>
            <Card
              title='Tổng đơn hàng'
              bordered={false}
              extra={<OrderAnimationIcon width={70} height={70} />}
              className='h-full'
            >
              <div className='text-[25px]'>
