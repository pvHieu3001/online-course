...
import HandleAnimationIcon from '@/page/components/icon/OrderIcon/Handle'
import PrepareSuccessAnimationIcon from '@/page/components/icon/OrderIcon/PrepareSuccess'
import DeliverAnimationIcon from '@/page/components/icon/OrderIcon/DeliverSuccess'
import PrepareAnimationIcon from '@/page/components/icon/OrderIcon/Prepare'
import DoneOrderAnimationIcon from '@/page/components/icon/OrderIcon/DoneOrder'
import OrderCancelAnimationIcon from '@/page/components/icon/OrderIcon/OrderCancel'
import DeliveringAnimationIcon from '@/page/components/icon/OrderIcon/Delivering'
import PickupAnimationIcon from '@/page/components/icon/OrderIcon/PickUp'
        ...
        <Col span={12}>
          <Card
            ...
            extra={
              <>
                {' '}
                <Badge color='green' text={dataItem?.order_status?.status} />
              </>
            }
          >
            ...
            <div className='flex justify-between border-solid border-b-[1px] border-b-[#eee] py-4'>
              ...
              <span className=''>
                {' '}
                <Tag color='#87d068'>{dataItem?.order_status?.status}</Tag>
              </span>
            </div>
          </Card>
        </Col>
        <Col span={12}>
          <Card title='Thông tin khách hàng' bordered={false}>
            ...
            <div className='flex justify-between border-solid border-b-[1px] border-b-[#eee] py-4'>
              ...
              <span className=''>
                {dataItem?.receiver_address}-{dataItem?.receiver_ward}-{dataItem?.receiver_district}-
                {dataItem?.receiver_pronvinces}
              </span>
            </div>
          </Card>
        </Col>
        ...
        <Col span={12}>
          <Card
            ...
            extra={
              <>
                {' '}
                <Tag color='#f50'>{dataItem?.payment_status}</Tag>
              </>
            }
          >
            <div className='flex justify-between border-solid border-b-[1px] border-b-[#eee] py-4'>
                      ...
                      <div className='relative flex space-x-3'>
                        ...
                        <div className='flex min-w-0 flex-1 justify-between space-x-4 '>
                          <div className='flex gap-3 items-center'>
                            <p className='text-sm text-gray-500 w-[130px]'>
                              {' '}
                            ...
                            <Button
                              onClick={() => changeStatusOrder(item.id)}
                              disabled={Boolean(getDateHistoreOrder(item.name)) || handleDisableButton(item.id)}
                              type='primary'
                            >
                              {Boolean(getDateHistoreOrder(item.name)) && 'Đã'} Xác nhận
                            </Button>
                          </div>
